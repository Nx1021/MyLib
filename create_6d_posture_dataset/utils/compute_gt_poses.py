...
if __name__ == "__main__":
     ...
     for path in folders:
          print(path)
          gtcomp = GtPostureComputer(path)
          cube = CubePosture(path)
          # gtcomp.compute_cube_pos(cube)
          # gtcomp.registration_with_marker()
          # cube = CubePosture(path)
          # cube_aruco_3d_C0_dict = read_json_as_int_ndarray(os.path.join(path, "cube_aruco_3d_C0_dict.json"))
          # # cube.load_cube_aruco_3d_C0_dict(cube_aruco_3d_C0_dict)
          # # cube.solve_cube_aruco_pos(cube.T_O2C0)
          # gtcomp.update_base_marker(cube.get_cube_aruco_C0_dict())
          # gtcomp.C0_aruco_3d_dict = cube.get_cube_aruco_C0_dict()
          Ts = gtcomp.registration_with_marker()
          Ts = np.load(os.path.join(gtcomp.directory, 'transforms.npy'))
          if Ts is not None:
               gtcomp.save_Ts_as_files(Ts)

          # cube = CubePosture(path)
          # cube.solve_cube_aruco_pos_by_images()
